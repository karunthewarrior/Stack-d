...
class block_color():
    ...
    def find_block(self,im):
        ...
        maskred = cv2.inRange(hsv_image,(5,50,0),(10,255,255)) #create mask of colours
        maskgreen = cv2.inRange(hsv_image, (20,30,0),(70,255,255)) #create mask of colours
        ...
        cv2.imshow("Segmented Image", maskred)
        cv2.waitKey(1)
        self.circle_list = []
        result = cv_image
    ...
    def find_depth(self,im):
        ...
        if(self.flag_block == True):
            ...
            if(len(self.position_list.x)==2):
                print("position:",(self.position_list.x[0]-self.position_list.x[1]),(self.position_list.y[0]-self.position_list.y[1]),((self.position_list.x[0]-self.position_list.x[1])**2+(self.position_list.y[0]-self.position_list.y[1])**2)**0.5)
                print("angle:",self.position_list.angle[0]-self.position_list.angle[1])
